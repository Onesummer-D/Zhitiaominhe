...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const pendingCases = casesRes.data.filter(c => c.status === 'pending' || c.status === 'mediating');
      setRecentCases(pendingCases.slice(0, 5));
    } catch (err) {
  ...
  return (
    <div className="dashboard-page" style={{ padding: '24px 32px', background: '#FAFBFD', minHeight: 'calc(100vh - 64px)'  }}>
      {/* ====== 新增：dashboard-layout 包裹开始 ====== */}
      <div className="dashboard-layout">
        {/* 欢迎区 */}
        <div className="welcome-section">
          <h1>上午好，李调解员</h1>
          <p>你今天有 <strong style={{ cursor: 'pointer', color: '#2C7473', textDecoration: 'underline' }} onClick={() => navigate('/cases')}>{stats.pending}</strong> 个待办案件</p>
        </div>
            ...
            <div className="pending-section">
              ...
              {loading ? (
                ...
                <div className="pending-list">
                  {recentCases.map((c) => (
                    <div key={c.id} className="pending-item" onClick={() => navigate('/mediation', { state: { caseData: c } })}>
